...
async def calculate_miner_metrics(response_event, agent, reward_result):
    ...
    for uid, response, status_code, timings in zip(response_event.uids, response_event.completions, response_event.status_codes, response_event.timings):
        ...
        prometheus_dict = {
            'reward': miner_metrics.reward,
            'rouge': miner_metrics.rouge,
            'relevance': miner_metrics.relevance,
            'reference_word_count': miner_metrics.reference_word_count,
            'response_word_count': miner_metrics.response_word_count,
            'challenge_word_count': miner_metrics.challenge_word_count,
            'availability': miner_metrics.availability,
            'response_time': miner_metrics.response_time,
        }

        # Call the updated function
        MinerMetrics.update_metrics_for_miner(uid_str, prometheus_dict)
